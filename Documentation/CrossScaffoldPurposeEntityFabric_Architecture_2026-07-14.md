...
## Implementert authority-, replika- og replay-slice 2026-07-15–16
...
- admission-bundet replika-store som persisterer en validert journal, leser tilbake eksakte canonical bytes og signerer ack først etter vellykket read-back,
- filbasert restart-test som viser reparasjon fra persisted journal når snapshot fortsatt er pre-commit.

Dette er kontrakt og testbevis, ikke en aktiv distribuert skrivebane. Quorum-evalueringen er ikke alene et certificate; certificate-fabrikken signerer bare en tilfredsstilt og kryptografisk bundet evaluering. Replika-store er foreløpig transportnøytral og frakoblet EntityAnchor. EntityAnchor mangler fortsatt bridge/outbox/live certificate-integrasjon og avviser derfor alle live requests med `requiredReplicaAcks > 0`.
...
- V0 har modeller, kontrakttester og en admission-bundet replika-store for durable replica acknowledgements, quorum-evaluering, authority-signert certificate og replay-range, men mangler bridgeadapter, durable outbox og certificate-persistens/integrasjon i live EntityAnchor.
...
2. Implementer bridgeadapteren rundt replika-store; adapteren frakter replay-materiale, mens store alene kan signere replica-ack etter validert persistens. Behold FlowCache kun som ytelses- og UX-optimalisering.
