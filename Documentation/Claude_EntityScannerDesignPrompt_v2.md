# Claude Prompt: Entity Scanner UI Design v2

Use this prompt with Claude when we want a grounded, architecture-safe UI proposal for `EntityScanner` in Binding.
...
Important: this is not a blank-slate speculative design exercise. You must stay grounded in the implementation model and contracts described below. You may propose new functionality, but only if it cleanly respects CellProtocol concepts, current ownership boundaries, and local-vs-remote separation.
...
The target experience is not a conference demo UI. It should feel like a high-trust Personal Co-Pilot surface for discovering nearby relevant people/entities and deciding whether to inspect published information, send an invitation, establish a signed contact relationship, or start a chat.
...
### 4. Identity exchange and persistence ground-truth

This part is critical.

In the current implementation, accepted contact is not only a visual “verified” state.

When contact is requested and accepted:
- the signed request payload includes `requesterIdentity`
- the signed acceptance payload includes `responderIdentity`
- those `Identity` objects carry public identity material / public keys
- the proof exchange is persisted as an encounter under:
  - `proofs.encounters.<encounterId>`
- the remote identity is also persisted locally under:
  - `relations.identities.<remoteIdentityUUID>`

So after contact establishment, the system has not merely “verified interest”:
- it has completed signed identity exchange
- it has persisted a durable local relationship record
- it has persisted encounter proof material

You must reflect this in the UX and architecture. Chat should not be described as unlocked merely after a vague “verification” state. It becomes available after signed identity exchange and local relation persistence.

### 5. Current relevance logic
...
### 6. What the new Entity Scanner experience should support
...
- let the user request or accept signed contact
- clearly show when signed identity exchange has happened
- clearly show when the relationship is saved locally
- let the user start or open a chat after that stronger relationship step

### 7. Important conceptual boundaries
...
- public profile data should come from a profile/public-directory style cell, not be invented inside the scanner
...
- a future richer `EntityRepresentation` model, but only clearly marked as future enhancement
...
- pretending that current ground-truth already has a richer entity-representation layer than it actually does
...
- how the scanner transitions from idle -> scanning -> visible matches -> selected entity -> invitation/contact -> signed relationship established -> chat
...
- signed relationship established / identity saved status
...
- how to distinguish:
  ...
  - signed relationship established result
...
- invitation CTA
- request contact / accept contact CTA
- signed relationship established state
- start/open chat CTA
- empty state
- scanning state
...
- whether it is local scanner state, public remote state, relation-persistence state, or chat-related state
...
### 9. Current ground-truth vs future enhancement
This is important.

For any relationship/persistence model you discuss, clearly distinguish:
- current ground-truth:
  - remote identity persisted under `relations.identities`
  - encounter proof persisted under `proofs.encounters`
- future enhancement:
  - a richer reusable `EntityRepresentation` or related abstraction

Do not blur these together.

### 10. CellProtocol discipline
...
8. Invitation/contact/relationship/chat flow
9. New functionality proposals
10. Current ground-truth vs future enhancement
11. CellProtocol discipline check
12. A short implementation roadmap

## Final instruction

Do not give me only visual moodboard language.
I need a design that could actually be implemented in Binding with:
- local Apple-backed scanner state
- current signed contact exchange
- current local relation persistence
- `CellConfiguration` composition
- additional absorbed cells where necessary
...
- This v2 prompt fixes the biggest gap from the earlier Claude pass: accepted contact is not only “verified”, but a durable identity-and-proof persistence event.
- It also leaves room for a future richer `EntityRepresentation` contract without misrepresenting today’s storage model.
